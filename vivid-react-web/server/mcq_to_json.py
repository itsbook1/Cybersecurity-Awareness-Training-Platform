...
def generate_questions(extracted_text):
    ...
    response = model.generate_content(
        f"""Create 5 questions with 3 incorrect and 1 correct multiple choice answers
...
def save_questions_to_csv(question_text, csv_path="questions.csv"):
    ...
    def parse_question_block(block):
        ...
        if "The correct answer is " in answer_line:
            ...
            explanation = (
                " ".join(correct_answer_part.split(" ")[1:]).strip()
                if " " in correct_answer_part
                else ""
            )
            ...
            correct_answer_text = next(
                (
                    ...
                    if choice.startswith(correct_answer_letter)
                ),
                "",
            )
        ...
        return (
            [question] + choices + [correct_answer, "Placeholder"]
        )  # Include explanation in correct answer
...
"""
# Example usage:
# For testing mcq_to_json independently
